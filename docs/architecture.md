...
| `src/lib/sources.mjs` | ソースアダプタ（`vegetan` / `commodity` / `retail` / `estat`）、リトライ付きfetch（テキスト/バイナリ）。estatはappIdをHTTP直前にのみ差し込みログ・エラーからscrub |
| `src/lib/estat.mjs` | e-Stat「青果物卸売市場調査」(00500226) の発見（getStatsList→最新公表年）と正規化。野菜4軸表・果実3軸表（消費地域・産地の複合軸）の両方に対応。産地シェア計算。**純関数**（フィクスチャで単体テスト） |
...
## 広告・SEO
- AdSense: `adsenseClientId` 設定時のみ head の配信スクリプトと各スロットを出力（未設定なら一切出力しない）。
- アフィリエイト: `config/site.json` の `affiliate` と品目の `buyKeyword` を利用して拡張可能な設計
  （現状は設定を保持する構造のみで、表示ロジックは未実装）。
