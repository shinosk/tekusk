...
運営者の X アカウントへ **1日1回** 自動投稿します（`scripts/post-x.mjs`）。

- **依存ゼロの OAuth 1.0a**: X API v2 の `POST /2/tweets` を OAuth 1.0a User Context で呼びます。
  HMAC-SHA1 署名・RFC 3986 パーセントエンコードは `node:crypto` のみで自作しています
  （`src/lib/social.mjs`。純関数なので `test/social.test.mjs` で署名の決定性を固定ベクトルで検証）。
- **必要な GitHub Secrets**: `X_API_KEY` / `X_API_SECRET` / `X_ACCESS_TOKEN` / `X_ACCESS_SECRET`。
  値はコミットもログ出力もしません。未設定なら「skipping（no credentials）」として何もせず `exit 0`。
- **冪等**: `data/social/x-state.json` に最後に投稿した集計日（`lastPostKey`）を保存し、
  新しい `meta.latestDate` のときだけ投稿します（同じ集計日に二重投稿しません）。
- **fail-safe**: API・ネットワーク失敗でも `exit 0`（日次ワークフローを壊さない）。`--strict` でのみ失敗を返します。
- **停止スイッチ**: `config/site.json` の `social.x.enabled` を `false` にすると投稿を止められます。
